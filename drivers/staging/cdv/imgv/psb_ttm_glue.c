...
/**
 * Ensure that the TTM object is only destroyed once all the PVR
 * allocations have been unmapped.
 */
void OnDestroyDRMCallback(void* pvData)
{
	struct psb_fpriv *psb_fp = (struct psb_fpriv*)pvData;
	if (psb_fp)
	{
...
int psb_open(struct inode *inode, struct file *filp)
{
	...
	PVRSRVSetDRMDestroyCallback(pvr_file_priv, OnDestroyDRMCallback, 
		psb_fp);
