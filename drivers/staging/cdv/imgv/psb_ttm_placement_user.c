			    ...
			    struct ttm_pl_rep *rep)
{
	...
	rep->sync_object_arg = _PSB_FENCE_TYPE_EXE;
}
			...
			struct ttm_buffer_object **p_bo)
{
	...
	size_t acc_size =
	    ttm_bo_acc_size(bdev, size, sizeof(struct ttm_bo_user_object));
			   ...
			   struct ttm_lock *lock, void *data)
{
	...
	if (atomic_read(&bo->cpu_writers) > 0) {
		ret = -EBUSY;
		goto out_err2;
	}
...
static int psb_ttm_bo_block_reservation(struct ttm_buffer_object *bo, bool interruptible,
                             bool no_wait)
{
        return ttm_bo_reserve(bo, interruptible, no_wait, false, NULL);
}
