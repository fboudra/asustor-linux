			       ...
			       uint64_t set_flags, uint64_t clr_flags)
{
	...
	ret = ttm_bo_reserve(bo, 1, 0, 1, &context->ticket);
	if (unlikely(ret != 0))
		goto out_unlock;
	...
	*/
	...
	context->fence_types |= _PSB_FENCE_TYPE_EXE;
			    ...
			    int no_wait, int interruptible)
{
	...
	if (unlikely(!ww_mutex_is_locked(&reloc_buffer->resv->lock))) {
		       ...
		       struct ttm_fence_object **fence_p)
{
	...
	if (ret) {
		...
		ttm_eu_backoff_reservation(&dev_priv->context.ticket, list);
		if (fence_p)
			*fence_p = NULL;
		return;
	}

	ttm_eu_fence_buffer_objects(&dev_priv->context.ticket, list, fence);
	if (!(fence_flags & DRM_PSB_FENCE_NO_USER)) {
			       ...
			       int ret)
{
	...
	if (ret) {
		ttm_eu_backoff_reservation(&context->ticket, list);
		ttm_eu_backoff_reservation(&context->ticket, 
			&context->kern_validate_list);
	}
...
*/
	...
	ret = ttm_eu_reserve_buffers(&context->ticket, 
		&context->validate_list);
