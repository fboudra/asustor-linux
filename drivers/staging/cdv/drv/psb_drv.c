...
static int psb_do_init(struct drm_device *dev)
{
	...
	ww_acquire_init(&dev_priv->context.ticket, &reservation_ww_class);
...
static void as_init_resource(struct drm_device *dev)
{
    if (reg_base == NULL)
    {
        DRM_INFO("[AS] GVD_MMADR=0x%X\n", (u32)pci_resource_start (dev->pdev, 0));

        reg_base = pci_iomap(dev->pdev, 0, 0);
        DRM_INFO("[AS] mapping to 0x%llX\n", (unsigned long long)reg_base);
    }
}

static void as_release_resource(struct drm_device *dev)
{
    if (reg_base)
    {
        pci_iounmap (dev->pdev, reg_base);
        reg_base = NULL;
    }
}

int IGFX_WRITE (struct drm_device *dev, u32 reg, u32 data)
{
    if (reg_base == NULL)
    {
        return (-ENOMEM);
    }
    iowrite32 (data, reg_base + reg);
//    DRM_INFO("[AS] GVD_MMADR=0x%llX offset=0x%X data=0x%X\n", (unsigned long long)reg_base, reg, data);
    return (0);
}

void as_hdmi_set_avi_infoframe(struct drm_device *dev)
{
    ...
    DRM_INFO("[AS] Set HDMI AVI infoframe...\n");
    port = VIDEO_DIP_PORT_B;

    IGFX_WRITE(dev, VIDEO_DIP_CTL, VIDEO_DIP_ENABLE | port |
           VIDEO_DIP_SELECT_AVI | VIDEO_DIP_FREQ_VSYNC);

    intel_dip_infoframe_csum(&avi_if);
    for (i = 0; i < sizeof(avi_if); i += 4) {
        IGFX_WRITE(dev, VIDEO_DIP_DATA, *data);
        data++;
    }
...
static int psb_driver_unload(struct drm_device *dev)
{
	...
	if (dev_priv) {
		...
		ww_acquire_fini(&dev_priv->context.ticket);
		kfree(dev_priv);
...
#if AS_ENABLE_HDMI_PATCH
    as_init_resource(dev);
    as_hdmi_set_avi_infoframe(dev);
#endif
...
static const struct file_operations drm_driver_fops = {
                 .owner = THIS_MODULE,
                 .open = psb_open,
                 .release = psb_release,
...
static struct drm_driver driver = {
	...
	.get_vblank_timestamp = psb_get_vblank_timestamp,
	.get_scanout_position = psb_get_crtc_scanoutpos,
	.firstopen = NULL,
