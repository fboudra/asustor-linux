...
/*
 * Called from drm generic code, retrieves the current
 * vblank timestamp of crtc on a given pipe.
 */
int psb_get_vblank_timestamp(struct drm_device *dev, int pipe, int *max_error,
...
/*
 * Called from drm generic code, retrieves the current
 * video scanout position of crtc on a given pipe.
 */
int psb_get_crtc_scanoutpos(struct drm_device *dev, int pipe, int *vpos,
			    int *hpos)
{
	struct drm_psb_private *dev_priv =
	    (struct drm_psb_private *) dev->dev_private;
	uint32_t pipeconf_reg = PIPEACONF;
	uint32_t pipedsl_reg = PIPEADSL;
	uint32_t vtotal_reg = VTOTAL_A;
	uint32_t vblank_reg = VBLANK_A;
	uint32_t reg_val = 0, vbl = 0, position = 0;
	...
	case 1:
		pipeconf_reg = PIPEBCONF;
		pipedsl_reg = PIPEBDSL;
		vtotal_reg = VTOTAL_B;
		vblank_reg = VBLANK_B;
		break;
	case 2:
		pipeconf_reg = PIPECCONF;
		pipedsl_reg = PIPECDSL;
		vtotal_reg = VTOTAL_C;
		vblank_reg = VBLANK_C;
		break;
	default:
	...
	if (!(reg_val & PIPEACONF_ENABLE)) {
		DRM_DEBUG_DRIVER("trying to get scanoutpos for disabled pipe %d\n", pipe);
		return -EINVAL;
	}

	/* Get vtotal. */
	vtotal = 1 + ((REG_READ(vtotal_reg) >> 16) & 0x1fff);

	/* Only query vertical scanout position from Display
	 * scan line register.
	 */
	position = REG_READ(pipedsl_reg);
