				 ...
				 uint32_t width, uint32_t height)
{
	...
	ret = psb_gtt_map_meminfo(dev, (IMG_HANDLE)(long)handle, &page_offset);
	if (ret) {
	...
	if (psb_intel_crtc->cursor_handle && psb_intel_crtc->cursor_handle != (void *)(long)handle)
		psb_gtt_unmap_meminfo(dev, psb_intel_crtc->cursor_handle);

	psb_intel_crtc->cursor_handle = (IMG_HANDLE)(long)handle;
