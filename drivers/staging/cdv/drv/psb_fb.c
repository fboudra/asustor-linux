...
extern unsigned long MRSTLFBGetPhysPage(struct MRSTLFB_BUFFER_TAG *pBuffer, int index);
...
static int psbfb_vm_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
	...
	int ret;
	unsigned long pfn;
	struct MRSTLFB_BUFFER_TAG *buffer = (struct MRSTLFB_BUFFER_TAG *)dev_priv->fb_reloc;
	...
	if (page_num > (psbfb->size >> PAGE_SHIFT))
	{
		page_num = psbfb->size >> PAGE_SHIFT;
	}
	for (i = 0; i < page_num; i++) {
		pfn = (MRSTLFBGetPhysPage(buffer, i) >> PAGE_SHIFT);
			 ...
			 struct drm_mode_fb_cmd2 *r)
{
	...
	IMG_HANDLE hKernelMemInfo = (IMG_HANDLE)(long)r->handles[0];
	struct drm_psb_private *dev_priv
...
static int psbfb_create(struct psb_fbdev * fbdev, struct drm_fb_helper_surface_size * sizes) 
{
	 ...
	 */
        ...
        mode_cmd.pitches[0] =  ALIGN(stride * ((32 /*mode_cmd.bpp*/ + 1) / 8), 64);
	...
	DRM_DEBUG("aligned_size=0x%x, pg->stolen_size=0x%lx", aligned_size, pg->stolen_size);
#if 0
	/*
	 * This is good for 64 bit.
	 */
	...
	if (buffer){
		psbfb->offset = MRSTLFBGetDevVAddr(buffer);
		DRM_DEBUG("psbfb->offset is the  DeviceVAddr 0x%x \n", psbfb->offset);
	}
	...
	if (buffer){
		...
		DRM_DEBUG("info->screen_base the FB CPU Vaddr %p for size 0x%x\n", info->screen_base, size);
	}
	else {
		info->screen_base = (char *)pg->vram_addr;
		memset(info->screen_base, 0, pg->stolen_size);
	}
	...
	DRM_DEBUG("fb mmio start 0x%lx \n", info->fix.mmio_start);
	DRM_DEBUG("fb mmio_len 0x%x \n", info->fix.mmio_len);
