...
//	    >> PAGE_SHIFT;
	...
	DRM_DEBUG("Set up %d CI stolen pages starting at 0x%08x, GTT offset %dK\n",
		  num_pages, pfn_base,(int) ((ttm_gtt_map - pg->gtt_map) * 4));
	for (i = 0; i < num_pages; ++i) {
		pte = psb_gtt_mask_pte(pfn_base + i, 0);
	...
	if (rar_stolen_size != 0) {
		...
		DRM_DEBUG("Set up %d RAR stolen pages starting at 0x%08x, GTT offset %dK\n",
			  num_pages, pfn_base,
			  (int)((ttm_gtt_map - pg->gtt_map + i) * 4));
				    ...
				    struct psb_gtt_hash_entry **hentry)
{
	...
	if (ret) {
		DRM_DEBUG("Cannot find entry pid=%lu\n", tgid);
		return ret;
	}
				      ...
				      struct psb_gtt_hash_entry **entry)
{
	...
	if (!ret) {
		DRM_DEBUG("Entry for tgid %lu exist, hentry %p\n",
			  tgid, hentry);
		*entry = hentry;
		spin_unlock(&mm->lock);
		return 0;
	...
	DRM_DEBUG("Entry for tgid %lu doesn't exist, will create it\n", tgid);
...
psb_gtt_mm_remove_ht_locked(struct psb_gtt_mm *mm, unsigned long tgid)
{
...
static int psb_gtt_mm_remove_free_ht_locked(struct psb_gtt_mm *mm, unsigned long tgid)
{
	struct psb_gtt_hash_entry *entry;
				    ...
				    struct psb_gtt_mem_mapping **entry)
{
	...
	if (!ret) {
		DRM_DEBUG("mapping entry for key %lu exists, entry %p\n",
			  key, mapping);
		*entry = mapping;
		spin_unlock(&mm->lock);
		return 0;
...
psb_gtt_mm_remove_mem_mapping_locked(struct drm_open_hash *ht, unsigned long key)
{
			       ...
			       struct drm_mm_node **node)
{
	...
	ret = psb_gtt_mm_get_ht_by_pid_locked(mm, tgid, &hentry);
	if (ret) {
		DRM_DEBUG("Cannot find entry for pid %lu\n", tgid);
		spin_unlock(&mm->lock);
		return ret;
	}
	spin_unlock(&mm->lock);
	...
	*node = tmp;
	...
	if (!hentry->count) {
		DRM_DEBUG("count of mapping entry is zero, tgid=%lu\n", tgid);
				...
				struct drm_mm_node **node)
{
	...
	if (!tmp_node)
	{
		DRM_DEBUG("psb_gtt_mm_alloc_mem - alloc failed\n");
		return -ENOMEM;
	}
	spin_lock(&mm->lock);
	ret = drm_mm_insert_node(&mm->base, tmp_node, pages, align, true);
	spin_unlock(&mm->lock);
	if (unlikely(ret)) {
		kfree(tmp_node);
		DRM_DEBUG("psb_gtt_mm_alloc_mem - Node insert failed\n");
		return -ENOMEM;
	}
			...
			unsigned long *offset)
{
	...
	DRM_DEBUG("Got psKernelMemInfo %p for handle 0x%lx\n",
		  psKernelMemInfo, (unsigned long)hKernelMemInfo);
			       ...
			       struct drm_file *file_priv)
{
	...
	return psb_gtt_map_meminfo(dev, arg->hKernelMemInfo,
...
int psb_gtt_unmap_pvr_memory(struct drm_device *dev, unsigned long hHandle, unsigned long ui32TaskId)
{
	struct drm_psb_private * dev_priv = (struct drm_psb_private *)dev->dev_private;
	struct psb_gtt_mm * mm = dev_priv->gtt_mm;
	struct psb_gtt * pg = dev_priv->pg;
