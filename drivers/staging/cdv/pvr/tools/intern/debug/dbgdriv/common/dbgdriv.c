...
void MonoOut(IMG_CHAR * pszString,IMG_BOOL bNewLine)
{
	...
	if (g_ui32Line == g_ui32MonoLines)
	{
		...
		HostMemSet((IMG_VOID *)(unsigned long)(DBGDRIV_MONOBASE + (160 * (g_ui32MonoLines - 1))),0,160);
	}
#endif	
}
...
static IMG_UINT32 WriteExpandingBuffer(PDBG_STREAM psStream,IMG_UINT8 * pui8InBuf,IMG_UINT32 ui32InBuffSize)
{
	...
	{
		PVR_DPF((PVR_DBG_ERROR, "WriteExpandingBuffer: buffer %lx is disabled", (IMG_UINTPTR_T) psStream));
		return(0);
	}
	...
	if (psStream->psCtrl->ui32Flags & DEBUG_FLAGS_NO_BUF_EXPANDSION)
	{
		...
		if (ui32Space < 32)
		{
			PVR_DPF((PVR_DBG_ERROR, "WriteExpandingBuffer: buffer %lx is full and isn't expandable", (IMG_UINTPTR_T) psStream));
			return(0);
		}
	}
	else
			...
			if (!ExpandStreamBuffer(psStream,ui32NewBufSize))
			{
				if (ui32Space < 32)
				{
					...
					else
					{
						...
						PVR_DPF((PVR_DBG_ERROR, "WriteExpandingBuffer: Unable to expand %lx. Out of memory.", (IMG_UINTPTR_T) psStream));
...
IMG_UINT32 IMG_CALLCONV DBGDrivWrite(PDBG_STREAM psMainStream,IMG_UINT8 * pui8InBuf,IMG_UINT32 ui32InBuffSize,IMG_UINT32 ui32Level)
{
	...
	if ((psStream->psCtrl->ui32OutMode & DEBUG_OUTMODE_STREAMENABLE) == 0)
	{
		PVR_DPF((PVR_DBG_ERROR, "DBGDrivWrite: buffer %lx is disabled", (IMG_UINTPTR_T) psStream));
		return(0);
	}

	if (ui32Space < 8)
	{
		PVR_DPF((PVR_DBG_ERROR, "DBGDrivWrite: buffer %lx is full", (IMG_UINTPTR_T) psStream));
		return(0);
	}
...
IMG_UINT32 IMG_CALLCONV DBGDrivRead(PDBG_STREAM psMainStream, IMG_BOOL bReadInitBuffer, IMG_UINT32 ui32OutBuffSize,IMG_UINT8 * pui8OutBuf)
{
	...
	{
		PVR_DPF((PVR_DBG_ERROR, "DBGDrivRead: buffer %lx is invalid", (IMG_UINTPTR_T) psMainStream));
		return(0);
	}
