						...
						IMG_DEV_PHYADDR	*psDevPAddr)
{
	...
	OSMemSet(psPTInfoList->PTPageCpuVAddr, 0, pMMUHeap->uiPTSize);
#endif
...
#if defined(PDUMP)
	{
		...
		PDUMPMALLOCPAGETABLE(&pMMUHeap->psMMUContext->psDeviceNode->sDevId, psPTInfoList->hPTPageOSMemHandle, 0, psPTInfoList->PTPageCpuVAddr, pMMUHeap->uiPTSize, ui32Flags, PDUMP_PT_UNIQUETAG);
		
		PDUMPMEMPTENTRIES(&pMMUHeap->sMMUAttrib, psPTInfoList->hPTPageOSMemHandle, psPTInfoList->PTPageCpuVAddr, pMMUHeap->uiPTSize, ui32Flags, IMG_TRUE, PDUMP_PT_UNIQUETAG, PDUMP_PT_UNIQUETAG);
	}
#endif
...
_DeferredFreePageTable (MMU_HEAP *pMMUHeap, IMG_UINT32 ui32PTIndex, IMG_BOOL bOSFreePT)
{
...
#if defined(PDUMP)
	{
		...
		{
			PDUMPFREEPAGETABLE(&pMMUHeap->psMMUContext->psDeviceNode->sDevId, ppsPTInfoList[ui32PTIndex]->hPTPageOSMemHandle, ppsPTInfoList[ui32PTIndex]->PTPageCpuVAddr, pMMUHeap->uiPTSize, ui32Flags, PDUMP_PT_UNIQUETAG);
		}
	}
#endif
...
MMU_Initialise (PVRSRV_DEVICE_NODE *psDeviceNode, MMU_CONTEXT **ppsMMUContext, IMG_DEV_PHYADDR *psPDDevPAddr)
{
	...
	{
		if (OSAllocPages(PVRSRV_HAP_WRITECOMBINE | PVRSRV_HAP_KERNEL_ONLY,
							(IMG_UINTPTR_T)SGX_MMU_PAGE_SIZE,
							SGX_MMU_PAGE_SIZE,
							&pvPDCpuVAddr,
	...
	PDUMPCOMMENT("Alloc page directory for new MMU context (PDDevPAddr == 0x%x)",
			sPDDevPAddr.uiAddr);
#else
	PDUMPCOMMENT("Alloc page directory for new MMU context, 64-bit arch detected (PDDevPAddr == 0x" DEVPADDR_FMT ")",
			sPDDevPAddr.uiAddr);
#endif
...

		{
			...
			if (!ppsPTInfoList[0])
			{
				PVR_DPF((PVR_DBG_MESSAGE, 
						 "MMU_UnmapPagesAndFreePTs: Invalid PT for alloc at VAddr:0x%X (VaddrIni:0x%X AllocPage:%u) PDIdx:%u PTIdx:%u",
						 sTmpDevVAddr.uiAddr, sDevVAddr.uiAddr,i, ui32PDIndex, ui32PTIndex ));
			...
			{
				PVR_DPF((PVR_DBG_MESSAGE, 
						 "MMU_UnmapPagesAndFreePTs: Page is already invalid for alloc at VAddr:0x%X (VAddrIni:0x%X AllocPage:%u) PDIdx:%u PTIdx:%u",
						 sTmpDevVAddr.uiAddr, sDevVAddr.uiAddr,i, ui32PDIndex, ui32PTIndex ));
			}
			...
			PDUMP_MMU_ATTRIB **ppsMMUAttrib)
{
	...
	pMMUHeap->uiPTSize = (1UL<<pMMUHeap->ui32PTBitWidth) * sizeof(IMG_UINT32);

	
	if(pMMUHeap->uiPTSize < 4 * sizeof(IMG_UINT32))
	{
		pMMUHeap->uiPTSize = 4 * sizeof(IMG_UINT32);
	}
	pMMUHeap->ui32PTNumEntriesAllocated = pMMUHeap->uiPTSize >> 2;
		   ...
		   IMG_DEV_VIRTADDR *psDevVAddr)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
		"MMU_Alloc: uSize=0x%lx, flags=0x%x, align=0x%x",
	...
	if (!bStatus)
	{
		...
		PVR_DPF((PVR_DBG_ERROR,"MMU_Alloc: Failed to alloc pagetable(s) for DevVAddr 0x%x from heap %s ID%d",
								psDevVAddr->uiAddr,
								pMMUHeap->psDevArena->pszName,
								pMMUHeap->psDevArena->ui32HeapID));
		if((uFlags & PVRSRV_MEM_USER_SUPPLIED_DEVVADDR) == 0)
...
MMU_Free (MMU_HEAP *pMMUHeap, IMG_DEV_VIRTADDR DevVAddr, IMG_UINT32 ui32Size)
{
	...
	PVR_DPF((PVR_DBG_MESSAGE, "MMU_Free: Freeing DevVAddr 0x%X from heap %s ID%d",
								DevVAddr.uiAddr,
								pMMUHeap->psDevArena->pszName,
								pMMUHeap->psDevArena->ui32HeapID));
	...
	PVR_DPF((PVR_DBG_ERROR,"MMU_Free: Couldn't free DevVAddr %X from heap %s ID%d (not in range of heap))",
							DevVAddr.uiAddr,
							pMMUHeap->psDevArena->pszName,
							pMMUHeap->psDevArena->ui32HeapID));
}
			 ...
			 IMG_UINT32 ui32MemFlags)
{
...
#if !defined(SUPPORT_SGX_MMU_DUMMY_PAGE)
	{
		...
		{
			PVR_DPF((PVR_DBG_ERROR, "MMU_MapPage: Page is already valid for alloc at VAddr:0x%X PDIdx:%d PTIdx:%u",
									DevVAddr.uiAddr,
									(unsigned int)(DevVAddr.uiAddr >> pMMUHeap->ui32PDShift),
									ui32Index ));
			PVR_DPF((PVR_DBG_ERROR, "MMU_MapPage: Page table entry value: 0x%08X", uTmp));
			PVR_DPF((PVR_DBG_ERROR, "MMU_MapPage: Physical page to map: 0x" DEVPADDR_FMT, DevPAddr.uiAddr));
		}
#if !defined(FIX_HW_BRN_31620)
				...
				IMG_HANDLE hUniqueTag)
{
	...
	for (i=0, uCount=0; uCount<uSize; i++, uCount+=pMMUHeap->ui32DataPageSize)
	{
		...
		PVR_DPF ((PVR_DBG_MESSAGE,
				 "MMU_MapScatter: devVAddr=%X, SysAddr=%lX, size=0x%x/0x%lx",
				  DevVAddr.uiAddr, sSysAddr.uiAddr, uCount, (unsigned long)uSize));
	}
			  ...
			  IMG_HANDLE hUniqueTag)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE, "MMU_MapPages: heap:%s, heap_id:%d devVAddr=%X, SysPAddr=%lX, size=0x%lx",
								pMMUHeap->psDevArena->pszName,
								pMMUHeap->psDevArena->ui32HeapID,
								DevVAddr.uiAddr, 
								SysPAddr.uiAddr,
			   ...
			   IMG_HANDLE         hUniqueTag)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			"MMU_MapShadow: DevVAddr:%X, Bytes:0x%lx, CPUVAddr:" UINTPTR_FMT,
			MapBaseDevVAddr.uiAddr,
			(unsigned long)uByteSize,
	...
	for (i=0; i<uByteSize; i+=ui32VAdvance)
	{
		...
		PVR_DPF ((PVR_DBG_MESSAGE,
				"Offset=0x%x: CpuVAddr=" UINTPTR_FMT ", CpuPAddr=" UINTPTR_FMT ", DevVAddr=%X, DevPAddr=" DEVPADDR_FMT,
				uOffset,
				...
				IMG_HANDLE hUniqueTag)
{
	...
	for(i=0; i<ui32PageCount; i++)
	{
		...
		if (!ppsPTInfoList[0])
		{
			PVR_DPF((PVR_DBG_ERROR, "MMU_UnmapPages: ERROR Invalid PT for alloc at VAddr:0x%X (VaddrIni:0x%X AllocPage:%u) PDIdx:%u PTIdx:%u",
									sTmpDevVAddr.uiAddr,
									sDevVAddr.uiAddr,
									i,
									ui32PDIndex,
		...
		{
			PVR_DPF((PVR_DBG_ERROR, "MMU_UnmapPages: Page is already invalid for alloc at VAddr:0x%X (VAddrIni:0x%X AllocPage:%u) PDIdx:%u PTIdx:%u",
									sTmpDevVAddr.uiAddr,
									sDevVAddr.uiAddr,
									i,
									ui32PDIndex,
...
MMU_GetPhysPageAddr(MMU_HEAP *pMMUHeap, IMG_DEV_VIRTADDR sDevVPageAddr)
{
	...
	if (!ppsPTInfoList[0])
	{
		PVR_DPF((PVR_DBG_ERROR,"MMU_GetPhysPageAddr: Not mapped in at 0x%x", sDevVPageAddr.uiAddr));
		sDevPAddr.uiAddr = 0;
		return sDevPAddr;
	}
...
PVRSRV_ERROR MMU_BIFResetPDAlloc(PVRSRV_SGXDEV_INFO *psDevInfo)
{
	...
	if(psLocalDevMemArena == IMG_NULL)
	{
		...
		eError = OSAllocPages(PVRSRV_HAP_WRITECOMBINE | PVRSRV_HAP_KERNEL_ONLY,
						      (IMG_UINTPTR_T)3 * SGX_MMU_PAGE_SIZE,
						      SGX_MMU_PAGE_SIZE,
