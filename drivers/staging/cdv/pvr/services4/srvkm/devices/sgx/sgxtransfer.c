...
#endif
{
	...
	if ((psKick->ui32Flags & SGXMKIF_TQFLAGS_KEEPPENDING) == 0UL)
	{
		...
		if ((psKick->ui32Flags & SGXMKIF_TQFLAGS_WAIT_VSYNC_PIPEA) || (psKick->ui32Flags & SGXMKIF_TQFLAGS_WAIT_VSYNC_PIPEB))
		{
			PVRSRV_KERNEL_SYNC_INFO* srcSyncInfo1 = NULL;
			PVRSRV_KERNEL_SYNC_INFO* srcSyncInfo2 = NULL;
			for (loop = 0; loop < psKick->ui32NumDstSync; loop++)
			{
				if (abDstSyncEnable[loop])
				{
					srcSyncInfo1 = (PVRSRV_KERNEL_SYNC_INFO*)psKick->ahDstSyncInfo[loop];
					break;
				}
			}
			for (loop = 0; loop < psKick->ui32NumSrcSync; loop++)
			{
				if (abSrcSyncEnable[loop])
				{
					srcSyncInfo2 = (PVRSRV_KERNEL_SYNC_INFO*)psKick->ahSrcSyncInfo[loop];
					break;
				}
			}
			if (srcSyncInfo1 && srcSyncInfo2 && srcSyncInfo1 != srcSyncInfo2)
			{
				PVRSRV_ERROR error = PVRSRVAddVSyncDependencyKM(srcSyncInfo1, srcSyncInfo2,
						(psKick->ui32Flags & SGXMKIF_TQFLAGS_WAIT_VSYNC_PIPEB ? IMG_TRUE : IMG_FALSE));
				if (error != PVRSRV_OK)
				{
					PVR_DPF((PVR_DBG_ERROR, "SGXSubmitTransferKM : Failed to add VSync dependency"));
				}
			}
			else
			{
				PVR_DPF((PVR_DBG_ERROR, "SGXSubmitTransferKM : Failed to add VSync dependency si1 = %p, si2 = %p", srcSyncInfo1, srcSyncInfo2));
			}
		}
