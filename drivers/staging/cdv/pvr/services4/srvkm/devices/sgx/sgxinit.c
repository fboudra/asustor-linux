								  ...
								  IMG_BOOL				bDumpSGXRegs)
{
	...
	if (bDumpSGXRegs)
	{
		PVR_DPF((PVR_DBG_ERROR,"SGX Register Base Address (Linear):   0x%lX", (IMG_UINTPTR_T)psDevInfo->pvRegsBaseKM));
		PVR_DPF((PVR_DBG_ERROR,"SGX Register Base Address (Physical): 0x%lX", psDevInfo->sRegsPhysBase.uiAddr));
...

	{
		...
		for (ui32LoopCounter = 0;
			 ...
			 ui32LoopCounter += 4)
		{
			PVR_LOG(("\t(HC-%ZX) 0x%08X 0x%08X 0x%08X 0x%08X", ui32LoopCounter * sizeof(*pui32HostCtlBuffer),
...

	{
		...
		for (ui32LoopCounter = 0;
			 ...
			 ui32LoopCounter += 4)
		{
			PVR_LOG(("\t(T3C-%ZX) 0x%08X 0x%08X 0x%08X 0x%08X", ui32LoopCounter * sizeof(*pui32TA3DCtlBuffer),
