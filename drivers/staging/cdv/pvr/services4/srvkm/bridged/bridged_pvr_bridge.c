					   ...
					   PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	psAllocDeviceMemOUT->eError =
		PVRSRVAllocDeviceMemKM(hDevCookieInt,
							   ...
							   psAllocDeviceMemIN->uiSize,
							   psAllocDeviceMemIN->uiAlignment,
							   &psMemInfo,
							   "" );
	...
	if (bUseShareMemWorkaround)
	{
		...
		psMemInfo->sShareMemWorkaround.uiOrigReqSize = psAllocDeviceMemIN->uiSize;
		psMemInfo->sShareMemWorkaround.uiOrigReqAlignment = psAllocDeviceMemIN->uiAlignment;
	}
	...
	if (psSrcKernelMemInfo->sShareMemWorkaround.bInUse)
	{
		...
		psMapDevMemOUT->eError =
			PVRSRVAllocDeviceMemKM(psSrcKernelMemInfo->sShareMemWorkaround.hDevCookieInt,
								   ...
								   psSrcKernelMemInfo->sShareMemWorkaround.uiOrigReqSize,
								   psSrcKernelMemInfo->sShareMemWorkaround.uiOrigReqAlignment,
								   &psDstKernelMemInfo,
								   "" );
					  ...
					  PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	IMG_UINTPTR_T ui32PageTableSize = 0;
					...
					PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	    ...
	    ((psGetMiscInfoIN->sMiscInfo.ui32StateRequest & PVRSRV_MISC_INFO_FREEMEM_PRESENT) != 0))
	{
		...
		ASSIGN_AND_EXIT_ON_ERROR(psGetMiscInfoOUT->eError,
				OSAllocMem(PVRSRV_OS_PAGEABLE_HEAP,
		                    (IMG_UINTPTR_T)psGetMiscInfoOUT->sMiscInfo.ui32MemoryStrLen,
