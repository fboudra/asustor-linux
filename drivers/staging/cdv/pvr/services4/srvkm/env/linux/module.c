...
#endif
#if defined(SUPPORT_DRI_DRM)
	psPrivateData->pvDRMDestroyCallback = NULL;
	psPrivateData->pvDRMDestroyCallbackData = NULL;
#endif
...
#if defined(SUPPORT_DRI_DRM)
void PVRSRVSetDRMDestroyCallback(
	void *pvPrivData,
	PVRSRVDRMDestroyCallbackFN pfnCallback,
	void* pvCallbackData)
{
	PVRSRV_FILE_PRIVATE_DATA* psPrivateData;
	...
	{
		psPrivateData->pvDRMDestroyCallback = pfnCallback;
		psPrivateData->pvDRMDestroyCallbackData = pvCallbackData;
	}
}
...
#endif
{
	...
	if (psPrivateData != IMG_NULL)
	{
		...
		if (PVRSRVProcessDisconnect(psPrivateData->ui32OpenPID))
		{
#if defined(SUPPORT_DRI_DRM)
			if (psPrivateData->pvDRMDestroyCallback)
			{
				PVRSRVDRMDestroyCallbackFN pfnCallback = 
					psPrivateData->pvDRMDestroyCallback;
				pfnCallback(psPrivateData->pvDRMDestroyCallbackData);
			}
#endif
		}
		gui32ReleasePID = 0;
