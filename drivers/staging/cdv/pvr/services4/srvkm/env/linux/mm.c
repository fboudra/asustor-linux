...
static IMG_VOID LinuxMMCleanup_MemAreas_ForEachCb(DEBUG_LINUX_MEM_AREA_REC *psCurrentRecord)
{
	...
	PVR_DPF((PVR_DBG_ERROR, "%s: BUG!: Cleaning up Linux memory area (%p), type=%s, size=%ld bytes",
				...
				(unsigned long)psCurrentRecord->psLinuxMemArea->uiByteSize));
                ...
                IMG_UINT32 ui32Line)
{
    ...
    pvRet = __vmalloc(uiBytes, GFP_KERNEL | __GFP_HIGHMEM | __GFP_ZERO, PGProtFlags);
...
LinuxMemArea *
NewVMallocLinuxMemArea(IMG_SIZE_T uiBytes, IMG_UINT32 ui32AreaFlags)
{
    LinuxMemArea *psLinuxMemArea;
    IMG_VOID *pvCpuVAddr;
    ...
    pvCpuVAddr = VMallocWrapper(uiBytes, ui32AreaFlags);
    if(!pvCpuVAddr)
    {
        goto failed;
    }
    ...
    ReservePages(pvCpuVAddr, uiBytes);
#endif

    psLinuxMemArea->eAreaType = LINUX_MEM_AREA_VMALLOC;
    psLinuxMemArea->uData.sVmalloc.pvVmallocAddress = pvCpuVAddr;
    psLinuxMemArea->uiByteSize = uiBytes;
    ...
    if(ui32AreaFlags & (PVRSRV_HAP_WRITECOMBINE | PVRSRV_HAP_UNCACHED))
        OSInvalidateCPUCacheRangeKM(psLinuxMemArea, pvCpuVAddr, uiBytes);
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,15))
static IMG_VOID
ReservePages(IMG_VOID *pvAddress, IMG_SIZE_T uiLength)
{
	IMG_VOID *pvPage;
	IMG_VOID *pvEnd = pvAddress + uiLength;
...
static IMG_VOID
UnreservePages(IMG_VOID *pvAddress, IMG_SIZE_T uiLength)
{
	IMG_VOID *pvPage;
	IMG_VOID *pvEnd = pvAddress + uiLength;
               ...
               IMG_UINT32 ui32Line)
{
    ...
    {
        case PVRSRV_HAP_CACHED:
	    pvIORemapCookie = (IMG_VOID *)IOREMAP(BasePAddr.uiAddr, uiBytes);
            break;
        case PVRSRV_HAP_WRITECOMBINE:
	    pvIORemapCookie = (IMG_VOID *)IOREMAP_WC(BasePAddr.uiAddr, uiBytes);
            break;
        case PVRSRV_HAP_UNCACHED:
            pvIORemapCookie = (IMG_VOID *)IOREMAP_UC(BasePAddr.uiAddr, uiBytes);
            break;
        default:
                       ...
                       IMG_UINT32 ui32AreaFlags)
{
    ...
    pvIORemapCookie = IORemapWrapper(BasePAddr, uiBytes, ui32AreaFlags);
    if(!pvIORemapCookie)
    {
        LinuxMemAreaStructFree(psLinuxMemArea);
        return NULL;
...
static IMG_BOOL
TreatExternalPagesAsContiguous(IMG_SYS_PHYADDR *psSysPhysAddr, IMG_SIZE_T uiBytes, IMG_BOOL bPhysContig)
{
	IMG_UINT32 ui32;
	IMG_UINT32 ui32AddrChk;
	IMG_UINT32 ui32NumPages = RANGE_TO_PAGES(uiBytes);
...
LinuxMemArea *NewExternalKVLinuxMemArea(IMG_SYS_PHYADDR *pBasePAddr, IMG_VOID *pvCPUVAddr, IMG_SIZE_T uiBytes, IMG_BOOL bPhysContig, IMG_UINT32 ui32AreaFlags)
{
    LinuxMemArea *psLinuxMemArea;
...
LinuxMemArea *
NewAllocPagesLinuxMemArea(IMG_SIZE_T uiBytes, IMG_UINT32 ui32AreaFlags)
{
    LinuxMemArea *psLinuxMemArea;
    ...
    ui32PageCount = RANGE_TO_PAGES(uiBytes);
...
LinuxMemAreaOffsetToPage(LinuxMemArea *psLinuxMemArea,
                         IMG_SIZE_T uiByteOffset)
{
    IMG_UINTPTR_T uiPageIndex;
    IMG_CHAR *pui8Addr;

    switch(psLinuxMemArea->eAreaType)
    {
        case LINUX_MEM_AREA_ALLOC_PAGES:
            uiPageIndex = PHYS_TO_PFN(uiByteOffset);
            return psLinuxMemArea->uData.sPageList.pvPageList[uiPageIndex];
 
        case LINUX_MEM_AREA_VMALLOC:
            pui8Addr = psLinuxMemArea->uData.sVmalloc.pvVmallocAddress;
            pui8Addr += uiByteOffset;
                   ...
                   IMG_SIZE_T uiBytes)
{
    ...
    PVR_ASSERT((uiByteOffset+uiBytes) <= psParentLinuxMemArea->uiByteSize);
...
IMG_CPU_PHYADDR
LinuxMemAreaToCpuPAddr(LinuxMemArea *psLinuxMemArea, IMG_SIZE_T uiByteOffset)
{
...
static void ProcSeqShowMemArea(struct seq_file *sfile,void* el)
{
	...
	if(el == PVR_PROC_SEQ_START_TOKEN) 
	{
        ...
        seq_printf( sfile,
#if !defined(DEBUG_LINUX_XML_PROC_FILES)
                       "%p       %-24s %p %p %ld %-5u %08x=(%s)\n",
#else
                       "<linux_mem_area>\n"
                       "\t<pointer>%p</pointer>\n"
                       "\t<type>%s</type>\n"
                       "\t<cpu_virtual>%p</cpu_virtual>\n"
                       "\t<cpu_physical>%p</cpu_physical>\n"
                       "\t<bytes>%ld</bytes>\n"
                       "\t<pid>%u</pid>\n"
                       "\t<flags>%08x</flags>\n"
                       ...
                       (IMG_PVOID)LinuxMemAreaToCpuPAddr(psRecord->psLinuxMemArea,0).uiAddr,
                       (unsigned long)psRecord->psLinuxMemArea->uiByteSize,
                       psRecord->pid,
                       psRecord->ui32Flags,
...
static void ProcSeqShowMemoryRecords(struct seq_file *sfile,void* el)
{
    ...
    if(psRecord->eAllocType != DEBUG_MEM_ALLOC_TYPE_KMEM_CACHE)
    {
		seq_printf( sfile,
                           ...
                           psRecord->pvCpuVAddr,
                           (IMG_PVOID)psRecord->ulCpuPAddr,
                           (unsigned long)psRecord->uiBytes,
                           psRecord->pid,
                           "NULL",
