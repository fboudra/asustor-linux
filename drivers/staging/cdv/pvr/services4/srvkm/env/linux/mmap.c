...
DestroyOffsetStruct(PKV_OFFSET_STRUCT psOffsetStruct)
{
    ...
    PVR_DPF((PVR_DBG_MESSAGE, "%s: Table entry: "
             "psLinuxMemArea=%p, CpuPAddr=%lx", __FUNCTION__,
...
#error "SPARC not supported"
	...
	{
	    IMG_UINTPTR_T pfn =  LinuxMemAreaToCpuPFN(psLinuxMemArea, uiPA);
	...
	for(uiPA = uiByteOffset; uiPA < uiByteEnd; uiPA += PAGE_SIZE)
	{
	    ...
	    pfn =  LinuxMemAreaToCpuPFN(psLinuxMemArea, uiPA);
...
MMapVCloseNoLock(struct vm_area_struct* ps_vma)
{
...
#if defined(DEBUG_LINUX_MMAP_AREAS)
    PVR_DPF((PVR_DBG_MESSAGE,
             "%s: psLinuxMemArea %p, CpuVAddr %p ui32MMapOffset %ld, ui32Mapped %d",
             __FUNCTION__,
             psOffsetStruct->psLinuxMemArea,
             LinuxMemAreaToCpuVAddr(psOffsetStruct->psLinuxMemArea),
             psOffsetStruct->uiMMapOffset,
	...
	if (psOffsetStruct->ui32RefCount != 0)
	{
	        PVR_DPF((PVR_DBG_MESSAGE, "%s: psOffsetStruct %p has non-zero reference count (ui32RefCount = %u). User mode address of start of mapping: 0x%lx", __FUNCTION__, psOffsetStruct, psOffsetStruct->ui32RefCount, psOffsetStruct->uiUserVAddr));
	}
...
PVRMMap(struct file* pFile, struct vm_area_struct* ps_vma)
{
    ...
    PVR_DPF((PVR_DBG_MESSAGE, "%s: Received mmap(2) request with uiMMapOffset 0x%08lx,"
                              " and uiByteSize %lx(0x%08lx)",
            __FUNCTION__,
            ps_vma->vm_pgoff,
            (unsigned long)uiByteSize, 
			(unsigned long)uiByteSize));
...
static void ProcSeqShowMMapRegistrations(struct seq_file *sfile, void *el)
{
	...
	if(el == PVR_PROC_SEQ_START_TOKEN) 
	{
        seq_printf( sfile,
                          ...
                          (unsigned long)g_uiTotalByteSize
                          );
		return;
	}
	...
	seq_printf( sfile,
#if !defined(DEBUG_LINUX_XML_PROC_FILES)
						"%p       %p %p %p " UINTPTR_FMT "   %ld   %-24s %-5u %-8s %08x(%s)\n",
#else
                        "<mmap_record>\n"
						"\t<pointer>%p</pointer>\n"
                        ...
                        (IMG_PVOID)LinuxMemAreaToCpuPAddr(psLinuxMemArea,0).uiAddr,
						psOffsetStruct->uiMMapOffset,
						(unsigned long)psLinuxMemArea->uiByteSize,
