        ...
        case PVRSRV_HAP_MULTI_PROCESS:
            ...
            if (eError != PVRSRV_OK)
            {
                PVR_DPF((PVR_DBG_ERROR,
                         "OSFreePages(ui32AllocFlags=0x%08X, ui32Bytes=%lx, "
                                        "pvCpuVAddr=%p, hOSMemHandle=%p) FAILED!",
                         ui32AllocFlags, 
						 (unsigned long)uiBytes, 
						 pvCpuVAddr, 
						 hOSMemHandle));
                return eError;
            }
...
IMG_CPU_PHYADDR
OSMemHandleToCpuPAddr(IMG_VOID *hOSMemHandle, IMG_SIZE_T uiByteOffset)
{
...
PVRSRV_ERROR OSInitEnvData(IMG_PVOID *ppvEnvSpecificData)
{
    ...
    eError = OSAllocMem(PVRSRV_OS_PAGEABLE_HEAP, (unsigned long)PVRSRV_MAX_BRIDGE_IN_SIZE + PVRSRV_MAX_BRIDGE_OUT_SIZE, 
              ...
              IMG_HANDLE *phOSMemHandle)
{
    ...
    switch(ui32MappingFlags & PVRSRV_HAP_MAPTYPE_MASK)
    {
        case PVRSRV_HAP_KERNEL_ONLY:
        {
        psLinuxMemArea = NewExternalKVLinuxMemArea(pBasePAddr, pvCPUVAddr, uiBytes, bPhysContig, ui32MappingFlags);
                ...
                IMG_HANDLE hOSMemHandle)
{
    ...
    switch(ui32MappingFlags & PVRSRV_HAP_MAPTYPE_MASK)
    {
        ...
        case PVRSRV_HAP_MULTI_PROCESS:
        {
            ...
            if (eError != PVRSRV_OK)
            {
                 PVR_DPF((PVR_DBG_ERROR, "%s(%p, %lx, 0x%08X, %p) FAILED!",
                          __FUNCTION__, 
						  pvCpuVAddr, 
						  (unsigned long)uiBytes,
                          ui32MappingFlags, 
...
IMG_BOOL OSAccessOK(IMG_VERIFY_TEST eVerification, IMG_VOID *pvUserPtr, IMG_SIZE_T uiBytes)
{
    ...
    {
	return access_ok(VERIFY_READ, pvUserPtr, uiBytes);
    }
    else
    {
        PVR_ASSERT(eVerification == PVR_VERIFY_WRITE);
	return access_ok(VERIFY_WRITE, pvUserPtr, uiBytes);
    }
}
...
typedef struct _sWrapMemInfo_
{
    ...
    IMG_INT iPageOffset;
#if defined(DEBUG)
    IMG_UINTPTR_T ulStartAddr;
    IMG_UINTPTR_T ulBeyondEndAddr;
    struct vm_area_struct *psVMArea;
#endif
} sWrapMemInfo;


static IMG_BOOL CPUVAddrToPFN(struct vm_area_struct *psVMArea, IMG_UINTPTR_T ulCPUVAddr, IMG_UINT32 *pulPFN, struct page **ppsPage)
{
...
{
    IMG_UINTPTR_T ulStartAddrOrig = (IMG_UINTPTR_T) pvCPUVAddr;
    IMG_SIZE_T ulAddrRangeOrig = uiBytes;
    IMG_UINTPTR_T ulBeyondEndAddrOrig = ulStartAddrOrig + ulAddrRangeOrig;
    IMG_UINTPTR_T ulStartAddr;
    IMG_UINTPTR_T ulAddrRange;
    IMG_UINTPTR_T ulBeyondEndAddr;
    IMG_UINTPTR_T ulAddr;
    IMG_INT i;
    struct vm_area_struct *psVMArea;
    ...
    {
        PVR_DPF((PVR_DBG_ERROR,
            "OSAcquirePhysPageAddr: Invalid address range (start %lx, length %lx)",
		ulStartAddrOrig, (unsigned long)ulAddrRangeOrig));
        goto error;
    }
...
IMG_VOID *FindMMapBaseVAddr(struct list_head *psMMapOffsetStructList,
							IMG_VOID *pvRangeAddrStart, IMG_UINTPTR_T uiLength)
{
	...
	list_for_each_entry(psOffsetStruct, psMMapOffsetStructList, sAreaItem)
	{
		...
		if(pvRangeAddrStart >= pvMinVAddr &&
		   uiLength <= psOffsetStruct->uiRealByteSize)
			return pvMinVAddr;
	}
...
{
	LinuxMemArea *psLinuxMemArea = (LinuxMemArea *)hOSMemHandle;
	IMG_SIZE_T uiAreaLength, uiAreaOffset = 0;
...
IMG_BOOL OSFlushCPUCacheRangeKM(IMG_HANDLE hOSMemHandle,
								IMG_VOID *pvRangeAddrStart,
								IMG_UINTPTR_T uiLength)
{
	
	return CheckExecuteCacheOp(hOSMemHandle, pvRangeAddrStart, uiLength,
