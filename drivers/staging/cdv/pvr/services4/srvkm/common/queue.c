...
void ProcSeqShowQueue(struct seq_file *sfile,void* el)
{
	...
	while (ui32ReadOffset != ui32WriteOffset)
	{
		...
		seq_printf(sfile, "%lx %lx  %5u  %6u  %lu  %5u   %2u   %2u    %lu  \n",
							(IMG_UINTPTR_T)psQueue,
							(IMG_UINTPTR_T)psCmd,
					 		psCmd->ui32ProcessID,
							psCmd->CommandType,
							(unsigned long)psCmd->uCmdSize,
							psCmd->ui32DevIndex,
							psCmd->ui32DstSyncCount,
							psCmd->ui32SrcSyncCount,
							(unsigned long)psCmd->uDataSize);
		{
			IMG_UINT32 i;
		...
		|| (ui32ReadOpsComplete != psWalkerObj->ui32ReadOpsPending))
		{
				...
				SYNCOPS_STALE(ui32ReadOpsComplete, psWalkerObj->ui32ReadOpsPending))
			{
				PVR_DPF((PVR_DBG_WARNING,
						"PVRSRVProcessCommand: Stale syncops psSyncData:0x%lx ui32WriteOpsComplete:0x%x ui32WriteOpsPending:0x%x",
