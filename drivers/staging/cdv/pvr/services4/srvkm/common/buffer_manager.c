				...
				BM_BUF				*pBuf)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  "AllocMemory (uSize=0x%lx, uFlags=0x%x, align=0x%x)",
			  (unsigned long)uSize, uFlags, uDevVAddrAlignment));
	...
	if(uFlags & PVRSRV_MEM_RAM_BACKED_ALLOCATION)
	{
		...
		{
			PVR_DPF((PVR_DBG_ERROR, "AllocMemory: RA_Alloc(0x%lx) FAILED", (unsigned long)uSize));
			return IMG_FALSE;
		}
	...
	else
	{
							...
							"Buffer Manager Mapping") != PVRSRV_OK)
		{
			PVR_DPF((PVR_DBG_ERROR, 
					"AllocMemory: OSAllocMem(0x%zx) FAILED", 
					sizeof(*pMapping)));
			return IMG_FALSE;
		}
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
				"AllocMemory: pMapping=%lx: DevV=%X CpuV=%lx CpuP=%lX uSize=0x%lx",
				(IMG_UINTPTR_T)pMapping,
				pMapping->DevVAddr.uiAddr,
				(IMG_UINTPTR_T)pMapping->CpuVAddr,
				pMapping->CpuPAddr.uiAddr,
				(unsigned long)pMapping->uSize));

	PVR_DPF ((PVR_DBG_MESSAGE,
				"AllocMemory: pBuf=%lx: DevV=%X CpuV=%lx CpuP=%lX uSize=0x%lx",
				(IMG_UINTPTR_T)pBuf,
			...
			BM_BUF *pBuf)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  "WrapMemory(psBMHeap=%lX, size=0x%lx, offset=0x%lx, bPhysContig=0x%x, pvCPUVAddr = 0x%lx, flags=0x%x)",
			  (IMG_UINTPTR_T)psBMHeap, 
			  (unsigned long)uSize, 
			  (unsigned long)uiBaseOffset, 
			  bPhysContig, 
			  (IMG_UINTPTR_T)pvCPUVAddr, uFlags));
	...
	{
		PVR_DPF((PVR_DBG_ERROR, "WrapMemory: OSAllocMem(0x%zx) FAILED",sizeof(*pMapping)));
		return IMG_FALSE;
	}
	...
	if(pvCPUVAddr)
	{
		...
		if (bPhysContig)
		{
			...
			{
				PVR_DPF((PVR_DBG_ERROR,	"WrapMemory: OSRegisterMem Phys=0x%lX, Size=%lx) failed",
					pMapping->CpuPAddr.uiAddr, 
					(unsigned long)pMapping->uSize));
				goto fail_cleanup;
			}
		...
		else
		{
			...
			if(OSRegisterDiscontigMem(pMapping->psSysAddr,
							...
							&pMapping->hOSMemHandle) != PVRSRV_OK)
			{
				PVR_DPF((PVR_DBG_ERROR,	"WrapMemory: OSRegisterDiscontigMem Size=%lx) failed",
					(unsigned long)pMapping->uSize));
				goto fail_cleanup;
			}
		}
	...
	else
	{
		if (bPhysContig)
		{
			...
			if(OSReservePhys(pMapping->CpuPAddr,
							 ...
							 &pMapping->hOSMemHandle) != PVRSRV_OK)
			{
				PVR_DPF((PVR_DBG_ERROR,	"WrapMemory: OSReservePhys Phys=0x%lX, Size=%lx) failed",
					pMapping->CpuPAddr.uiAddr, 
					(unsigned long)pMapping->uSize));
				goto fail_cleanup;
			}
		...
		else
		{
			...
			if(OSReserveDiscontigPhys(pMapping->psSysAddr,
							 ...
							 &pMapping->hOSMemHandle) != PVRSRV_OK)
			{
				PVR_DPF((PVR_DBG_ERROR,	"WrapMemory: OSReserveDiscontigPhys Size=%lx) failed",
					(unsigned long)pMapping->uSize));
				goto fail_cleanup;
			}
		}
	...
	{
		PVR_DPF((PVR_DBG_ERROR,
				"WrapMemory: DevMemoryAlloc(0x%lx) failed",
				(unsigned long)pMapping->uSize));
		goto fail_cleanup;
	}
	...
	PVR_DPF ((PVR_DBG_MESSAGE, "DevVaddr.uiAddr=%X", DevVAddr.uiAddr));
	PVR_DPF ((PVR_DBG_MESSAGE,
				"WrapMemory: DevV=%X CpuP=%lX uSize=0x%lx",
				pMapping->DevVAddr.uiAddr, 
				pMapping->CpuPAddr.uiAddr, 
				(unsigned long)pMapping->uSize));
	PVR_DPF ((PVR_DBG_MESSAGE,
...
FreeBuf (BM_BUF *pBuf, IMG_UINT32 ui32Flags, IMG_BOOL bFromAllocator)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			"FreeBuf: pBuf=0x%lx: DevVAddr=%X CpuVAddr=0x%lx CpuPAddr=%lX",
			...
			BM_HANDLE			*phBuf)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
		  "BM_Alloc (uSize=0x%lx, uFlags=0x%x, uDevVAddrAlignment=0x%x)",
			(unsigned long)uSize, uFlags, uDevVAddrAlignment));
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
		  "BM_Alloc (uSize=0x%lx, uFlags=0x%x)",
		  (unsigned long)uSize, uFlags));
			...
			BM_HANDLE *phBuf)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
		  "BM_Wrap (uSize=0x%lx, uOffset=0x%lx, bPhysContig=0x%x, pvCPUVAddr=0x%lx, uFlags=0x%x)",
	...
	if (bPhysContig)
	{
		if (!ValidSysPAddrRangeForDev(psBMContext->psDeviceNode, *psSysAddr, WRAP_MAPPING_SIZE(uiSize, ui32Offset)))
		{
	...
	{
		IMG_SIZE_T uiHostPageSize = HOST_PAGESIZE();

		if (!ValidSysPAddrArrayForDev(psBMContext->psDeviceNode, psSysAddr, WRAP_PAGE_COUNT(uiSize, ui32Offset, ui32HostPageSize), ui32HostPageSize))
		{
	...
	if(pBuf)
	{
		IMG_SIZE_T uiMappingSize = HOST_PAGEALIGN (uiSize + uiOffset);

		
		if(pBuf->pMapping->uSize == uiMappingSize && (pBuf->pMapping->eCpuMemoryOrigin == hm_wrapped ||
													pBuf->pMapping->eCpuMemoryOrigin == hm_wrapped_virtaddr))
		{
			PVR_DPF((PVR_DBG_MESSAGE,
					"BM_Wrap (Matched previous Wrap! uSize=0x%lx, uOffset=0x%lx, SysAddr=%lx)",
					(unsigned long)uiSize, 
					(unsigned long)uiOffset, sHashAddress.uiAddr));
	...
	if (WrapMemory (psBMHeap, uiSize, uiOffset, bPhysContig, psSysAddr, pvCPUVAddr, uFlags, pBuf) != IMG_TRUE)
	{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			"BM_Wrap (uSize=0x%lx, uFlags=0x%x, devVAddr=%X)",
			(unsigned long)uiSize, uFlags, pBuf->DevVAddr.uiAddr));
		...
		IMG_UINT32 ui32Flags)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE, "BM_Free (h=0x%lx)", (IMG_UINTPTR_T)hBuf));
...
BM_HandleToCpuVaddr (BM_HANDLE hBuf)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
				"BM_HandleToCpuVaddr(h=0x%lx)=0x%lx",
...
BM_HandleToDevVaddr (BM_HANDLE hBuf)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE, "BM_HandleToDevVaddr(h=0x%lx)=%X", (IMG_UINTPTR_T)hBuf, pBuf->DevVAddr.uiAddr));
...
BM_HandleToSysPaddr (BM_HANDLE hBuf)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE, "BM_HandleToSysPaddr(h=0x%lx)=%lX", (IMG_UINTPTR_T)hBuf, pBuf->CpuPAddr.uiAddr));
...
BM_HandleToOSMemHandle(BM_HANDLE hBuf)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
				"BM_HandleToOSMemHandle(h=0x%lx)=0x%lx",
                              ...
                              IMG_HANDLE *phOSMemHandle)
{
	...
	if (gXProcWorkaroundShareData[gXProcWorkaroundShareIndex].ui32RefCount > 0)
	{
		...
		if (uiSize != gXProcWorkaroundShareData[gXProcWorkaroundShareIndex].uiSize)
		{
			PVR_DPF((PVR_DBG_ERROR,
	...
	else
	{
		if (psArena != IMG_NULL)
		{
			...
			{
				PVR_DPF((PVR_DBG_ERROR, "XProcWorkaroundAllocShareable: RA_Alloc(0x%zx) FAILED", uiSize));
				return PVRSRV_ERROR_OUT_OF_MEMORY;
			}
		...
		gXProcWorkaroundShareData[gXProcWorkaroundShareIndex].uiSize = uiSize;
		...
		if (gXProcWorkaroundShareData[ui32SI].psArena != IMG_NULL)
		{
			...
			if (gXProcWorkaroundShareData[ui32SI].pvCpuVAddr != IMG_NULL)
			{
				OSUnReservePhys(gXProcWorkaroundShareData[ui32SI].pvCpuVAddr,
								gXProcWorkaroundShareData[ui32SI].uiSize,
			  ...
			  IMG_UINTPTR_T *pBase)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  "BM_ImportMemory (pBMContext=0x%lx, uRequestSize=0x%lx, uFlags=0x%x, uAlign=0x%lx)",
			  (IMG_UINTPTR_T)pBMContext, 
			  (unsigned long)uRequestSize, 
			  uFlags, 
			  (unsigned long)uDevVAddrAlignment));
...
#define MAX(a,b) ((a) > (b) ? (a) : (b))
		...
		{
			PVR_DPF((PVR_DBG_ERROR,
					"BM_ImportMemory: XProcWorkaroundAllocShareable(0x%lx) failed",
	...
	if(pBMHeap->ui32Attribs & PVRSRV_BACKINGSTORE_SYSMEM_NONCONTIG)
	{
		...
		if (OSAllocPages(ui32Attribs,
						 (long unsigned)uPSize,
						 pBMHeap->sDevArena.ui32DataPageSize,
						 (IMG_VOID **)&pMapping->CpuVAddr,
						 &pMapping->hOSMemHandle) != PVRSRV_OK)
		{
			PVR_DPF((PVR_DBG_ERROR,
					"BM_ImportMemory: OSAllocPages(0x%lx) failed",
					(unsigned long)uPSize));
			goto fail_mapping_alloc;
		}
	...
	else if(pBMHeap->ui32Attribs & PVRSRV_BACKINGSTORE_LOCALMEM_CONTIG)
	{
					   ...
					   (IMG_UINTPTR_T *)&sSysPAddr.uiAddr))
		{
			PVR_DPF((
			PVR_DBG_ERROR, 
			"BM_ImportMemory: RA_Alloc(0x%lx) FAILED", 
			(unsigned long)uPSize));
	...
	{
		PVR_DPF((PVR_DBG_ERROR,
				"BM_ImportMemory: DevMemoryAlloc(0x%lx) failed",
				(unsigned long)pMapping->uSize));
		goto fail_dev_mem_alloc;
	}
...
BM_FreeMemory (IMG_VOID *h, IMG_UINTPTR_T _base, BM_MAPPING *psMapping)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  "BM_FreeMemory (h=0x%lx, base=0x%lx, psMapping=0x%lx)",
	...
	PVR_DPF((PVR_DBG_MESSAGE,
			"..BM_FreeMemory (h=0x%lx, base=0x%lx)",
