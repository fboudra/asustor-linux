...
{
	PVR_DPF((PVR_DBG_MESSAGE,"WriteHWReg Base:%lx, Offset: %x, Value %x",
...
IMG_EXPORT
IMG_BOOL IMG_CALLCONV PVRSRVProcessDisconnect(IMG_UINT32	ui32PID)
{
	return PVRSRVPerProcessDataDisconnect(ui32PID);
}
														...
														IMG_SIZE_T *puiBufSize, IMG_BOOL bSave)
{
	...
	while (RA_GetNextLiveSegment(hArena, &sSegDetails))
	{
		...
		else
		{
			...
			PVR_DPF((
				PVR_DBG_MESSAGE, 
				"PVRSRVSaveRestoreLiveSegments: Base %lx size %08lx", 
				sSegDetails.sCpuPhyAddr.uiAddr, 
				(unsigned long)sSegDetails.uiSize));
