...
_InsertResourceSpan (RA_ARENA *pArena, IMG_UINTPTR_T base, IMG_SIZE_T uSize)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  "RA_InsertResourceSpan: arena='%s', base=0x%lx, size=0x%lx",
			  pArena->name, base, (unsigned long)uSize));
	...
	while (uIndex < FREE_TABLE_LIMIT)
	{
		if (pArena->aHeadFree[uIndex]!=IMG_NULL)
		{
			...
			while (pBT!=IMG_NULL)
			{
				...
				PVR_DPF ((PVR_DBG_MESSAGE,
						  "RA_AttemptAllocAligned: pBT-base=0x%lx "
						  "pBT-size=0x%lx alignedbase=0x%lx size=0x%lx",
						pBT->base, (unsigned long)pBT->uSize, 
						aligned_base, (unsigned long)uSize));
		   ...
		   IMG_VOID *pImportHandle)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  "RA_Create: name='%s', base=0x%lx, uSize=0x%lx, alloc=0x%lx, free=0x%lx",
			  name, base, (unsigned long)uSize, 
			  (IMG_UINTPTR_T)imp_alloc, (IMG_UINTPTR_T)imp_free));
...
RA_Delete (RA_ARENA *pArena)
{
	...
	while (pArena->pHeadSegment != IMG_NULL)
	{
		...
		if (pBT->type != btt_free)
		{
			...
			PVR_DPF ((PVR_DBG_ERROR,"RA_Delete: base = 0x%lx size=0x%lx", pBT->base, (unsigned long)pBT->uSize));
		}
	...
	if (pArena != IMG_NULL)
	{
		while (pArena->pHeadSegment != IMG_NULL)
		{
			...
			if (pBT->type != btt_free)
			{
				...
				PVR_DPF ((PVR_DBG_ERROR,"RA_TestDelete: base = 0x%lx size=0x%lx", 
						pBT->base, (unsigned long)pBT->uSize));
				return IMG_FALSE;
			}
		}
...
RA_Add (RA_ARENA *pArena, IMG_UINTPTR_T base, IMG_SIZE_T uSize)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  "RA_Add: name='%s', base=0x%lx, size=0x%lx", 
			  pArena->name, base, (unsigned long)uSize));
		  ...
		  IMG_UINTPTR_T *base)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  "RA_Alloc: arena='%s', size=0x%lx(0x%lx), alignment=0x%x, offset=0x%x",
		   pArena->name, (unsigned long)uSize, (unsigned long)uRequestSize, 
		...
		if (bResult)
		{
			...
			if (pBT == IMG_NULL)
			{
				...
				pArena->pImportFree(pArena->pImportHandle, import_base,
									psImportMapping);
				PVR_DPF ((PVR_DBG_MESSAGE,
						  "RA_Alloc: name='%s', size=0x%lx failed!",
						  pArena->name, (unsigned long)uSize));
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  ...
			  pArena->name, (unsigned long)uSize, *base, bResult));
...
static void RA_ProcSeqShowInfo(struct seq_file *sfile, void* el)
{
	...
	IMG_INT off = (IMG_INT)(unsigned long)el;

	switch (off)
	{
	case 1:
		seq_printf(sfile, "quantum\t\t\t%lx\n", (unsigned long)pArena->uQuantum);
		break;
	case 2:
		seq_printf(sfile, "import_handle\t\t%08u\n", (IMG_UINT)(unsigned long)pArena->pImportHandle);
		break;
#ifdef RA_STATS
	case 3:
		seq_printf(sfile,"span count\t\t%lx\n", (unsigned long)pArena->sStatistics.uSpanCount);
		break;
	case 4:
		seq_printf(sfile, "live segment count\t%lx\n", (unsigned long)pArena->sStatistics.uLiveSegmentCount);
		break;
	case 5:
		seq_printf(sfile, "free segment count\t%lx\n", (unsigned long)pArena->sStatistics.uFreeSegmentCount);
		break;
	case 6:
		seq_printf(sfile, "free resource count\t%lx (0x%u)\n",
							(unsigned long)pArena->sStatistics.uFreeResourceCount,
							(IMG_UINT)pArena->sStatistics.uFreeResourceCount);
		break;
	case 7:
		seq_printf(sfile, "total allocs\t\t%lx\n", (unsigned long)pArena->sStatistics.uCumulativeAllocs);
		break;
	case 8:
		seq_printf(sfile, "total frees\t\t%lx\n", (unsigned long)pArena->sStatistics.uCumulativeFrees);
		break;
	case 9:
		seq_printf(sfile, "import count\t\t%lx\n", (unsigned long)pArena->sStatistics.uImportCount);
		break;
	case 10:
		seq_printf(sfile, "export count\t\t%lx\n", (unsigned long)pArena->sStatistics.uExportCount);
		break;
#endif
	}
...
static void* RA_ProcSeqOff2ElementInfo(struct seq_file * sfile, loff_t off)
{
	...
	if(off <= 1)
#endif
		return (void*)(unsigned long)(IMG_INT)(off+1);
	return 0;
}
...
static void RA_ProcSeqShowRegs(struct seq_file *sfile, void* el)
{
	...
	if (pBT)
	{
		seq_printf(sfile, "%08x %8x %4s %08x\n",
			       ...
			       (IMG_UINT)(unsigned long)pBT->psMapping);
	}
}
							...
							IMG_UINT32 *pui32StrLen)
{
	...
	CHECK_SPACE(ui32StrLen);
	i32Count = OSSNPrintf(pszStr, 100, "  allocCB=%p freeCB=%p handle=%p quantum=%lu\n",
							 pArena->pImportAlloc,
							 pArena->pImportFree,
	...
	{
		CHECK_SPACE(ui32StrLen);
		i32Count = OSSNPrintf(pszStr, 100, "\tbase=0x%x size=0x%lx type=%s ref=%p\n",
											 (IMG_UINT32) pBT->base,
											 (unsigned long)pBT->uSize,
											 _BTType(pBT->type),
											 pBT->psMapping);
		UPDATE_SPACE(pszStr, i32Count, ui32StrLen);
	}
								...
								IMG_UINT32 *pui32StrLen)
{
	...
	IMG_INT32	i32Count;
	CHECK_SPACE(ui32StrLen);
	i32Count = OSSNPrintf(pszStr, 100, "Bytes free: Arena %-30s: %lu (0x%lx)\n", 
			pArena->name,
			(unsigned long)pArena->sStatistics.uFreeResourceCount,
			(unsigned long)pArena->sStatistics.uFreeResourceCount);
	UPDATE_SPACE(pszStr, i32Count, ui32StrLen);
