...
typedef struct PVRSRV_DISPLAYCLASS_INFO_TAG
{
	...
	PVRSRV_QUEUE_INFO					*psVSyncQueue;
...
PVRSRV_ERROR PVRSRVAddVSyncDependencyKM(PVRSRV_KERNEL_SYNC_INFO* psSrcSyncInfo1,
										PVRSRV_KERNEL_SYNC_INFO* psSrcSyncInfo2,
										IMG_BOOL bUsePipeB)
{
	PVRSRV_ERROR eError;
	DISPLAYCLASS_VSYNC_COMMAND *psVSyncCmd;
	IMG_UINT16 ui16VSyncCommandID = DC_VSYNC_COMMAND;
	PVRSRV_COMMAND *psCommand;
	SYS_DATA *psSysData;
	PVRSRV_DISPLAYCLASS_INFO *psDCInfo;
	PVRSRV_DEVICE_NODE *psDeviceNode;
	IMG_UINT32 ui32Pipe = ( bUsePipeB ? CRTC_PIPEB : CRTC_PIPEA );
	PVRSRV_KERNEL_SYNC_INFO* syncList[] = {psSrcSyncInfo1, psSrcSyncInfo2};

	if(!psSrcSyncInfo1 || !psSrcSyncInfo2)
	{
		PVR_DPF((PVR_DBG_ERROR,"PVRSRVWaitForVSyncKM: Invalid parameters"));
		return PVRSRV_ERROR_INVALID_PARAMS;
	}
	...
	{
		PVR_DPF((PVR_DBG_ERROR,"PVRSRVWaitForVSyncKM: Display device not registered"));
		return PVRSRV_ERROR_INIT_FAILURE;
	}

	psDCInfo = (PVRSRV_DISPLAYCLASS_INFO*)psDeviceNode->pvDevice;

	if (!psDCInfo->psVSyncQueue)
	{
		eError = PVRSRVCreateCommandQueueKM(1024, &psDCInfo->psVSyncQueue);
		if(eError != PVRSRV_OK)
		{
			PVR_DPF((PVR_DBG_ERROR,"PVRSRVWaitForVSyncKM: Failed to create CmdQueue"));
			goto Exit;
		}
	}

	eError = PVRSRVInsertCommandKM (psDCInfo->psVSyncQueue,
									&psCommand,
									psDCInfo->ui32DeviceID,
									ui16VSyncCommandID,
									0,
									...
									2,
									syncList,
									sizeof(DISPLAYCLASS_VSYNC_COMMAND));
	if(eError != PVRSRV_OK)
	{
		PVR_DPF((PVR_DBG_ERROR,"PVRSRVWaitForVSyncKM: Failed to get space in queue"));
		goto Exit;
	}

	psVSyncCmd = (DISPLAYCLASS_VSYNC_COMMAND*)psCommand->pvData;

	psVSyncCmd->hExtDevice = psDCInfo->hExtDevice;
	psVSyncCmd->ui32Pipe = ui32Pipe;
	eError = PVRSRVSubmitCommandKM (psDCInfo->psVSyncQueue, psCommand);
	if (eError != PVRSRV_OK)
	{
		PVR_DPF((PVR_DBG_ERROR,"PVRSRVWaitForVSyncKM: Failed to submit command"));
		goto Exit;
	}
	...
	if (eError != PVRSRV_OK)
	{
		PVR_DPF((PVR_DBG_ERROR,"PVRSRVWaitForVSyncKM: Failed to schedule MISR"));
		goto Exit;
	}
