...
MRST_BOOL MRSTLFBIsPipeEnabled(MRSTLFB_DEVINFO * psDevinfo, IMG_UINT32 ui32Pipe)
{
	...
	return (dev_priv->pipeEnabled & (1 << ui32Pipe));
#else
	return MRST_FALSE;
#endif
}
...
void MRSTLFBEnableVSyncInterrupt(MRSTLFB_DEVINFO * psDevinfo, IMG_UINT32 ui32Pipe)
{
	...
	struct drm_psb_private *dev_priv =
			(struct drm_psb_private *) psDevinfo->psDrmDevice->dev_private;
	if (!psDevinfo->abVSyncEnabled[ui32Pipe])
	{
		psDevinfo->ui32FlipEventEnabled |= (1 << ui32Pipe);
		dev_priv->vblanksEnabledForFlips = true;
		psb_enable_vblank(psDevinfo->psDrmDevice, ui32Pipe);
		psDevinfo->abVSyncEnabled[ui32Pipe] = MRST_TRUE;
	}
#endif
}
...
void MRSTLFBDisableVSyncInterrupt(MRSTLFB_DEVINFO * psDevinfo, IMG_UINT32 ui32Pipe)
{
	...
	{
		psDevinfo->ui32FlipEventEnabled &= ~(1 << ui32Pipe);
		if (!psDevinfo->ui32FlipEventEnabled) 
		{
			dev_priv->vblanksEnabledForFlips = false;
		}
		if (!dev->vblank_enabled[ui32Pipe])
		{
			psb_disable_vblank(psDevinfo->psDrmDevice, ui32Pipe);
			psDevinfo->abVSyncEnabled[ui32Pipe] = MRST_FALSE;
		}
	}
#endif
}
...
{
	int dspbase = (pipe == CRTC_PIPEA ? DSPABASE : DSPBBASE);
	int dspsurf = (pipe == CRTC_PIPEA ? DSPASURF : DSPBSURF);
	int dspstride = (pipe == CRTC_PIPEA ? DSPASTRIDE : DSPBSTRIDE);

	if (IS_CDV(psDevInfo->psDrmDevice)) {
		MRSTLFBVSyncWriteReg(psDevInfo, dspstride, stride);
		MRSTLFBVSyncWriteReg(psDevInfo, dspbase, offset);
