...
#define MAX_VSYNC_DEPENDENCIES    16
#define MAX_PIPES                 2
#define CRTC_PIPEA                0
#define CRTC_PIPEB                1
#define VSYNC_REQUIRED_SWAPCHAIN  (1<<0)
#define VSYNC_REQUIRED_DEP        (1<<1)
...
typedef struct MRSTLFB_VSYNC_DEPENDENCY_ITEM_TAG
{
	MRST_HANDLE      hCmdComplete;
...
typedef struct MRSTLFB_LAST_FLIP_TAG
{
...
typedef struct MRSTLFB_DEVINFO_TAG
{
	...
	MRSTLFB_SWAPCHAIN      *apsCurrentSwapChain[MAX_PIPES];
	...
	MRSTLFB_VSYNC_DEPENDENCY_ITEM asVSyncDependencies[MAX_VSYNC_DEPENDENCIES];
	IMG_UINT32 ui32VSyncDependencies;
	MRST_BOOL abVSyncEnabled[MAX_PIPES];
	IMG_UINT32 abVSyncEventRequired[MAX_PIPES];
	MRSTLFB_LAST_FLIP asLastFlips[MAX_PIPES];
	IMG_UINT32 ui32FlipEventEnabled;
	IMG_UINT32 aui32CurrentOffset[MAX_PIPES];
	MRST_BOOL bIsCloneMode;
	MRST_BOOL bDisplayModeChecked;
}  MRSTLFB_DEVINFO;
...
MRST_BOOL MRSTLFBIsPipeEnabled(MRSTLFB_DEVINFO * psDevinfo, IMG_UINT32 ui32Pipe);
void MRSTLFBEnableVSyncInterrupt(MRSTLFB_DEVINFO *psDevInfo, IMG_UINT32 ui32Pipe);
void MRSTLFBDisableVSyncInterrupt(MRSTLFB_DEVINFO *psDevInfo, IMG_UINT32 ui32Pipe);

void MRSTLFBFlipToSurface(MRSTLFB_DEVINFO *psDevInfo, unsigned long uiAddr, int stride, unsigned int offset, int pipe);
