...
#define ENABLE_VSYNC_DEBUG 0

#if (ENABLE_VSYNC_DEBUG)
#define VSYNC_DEBUG(_fmt, _arg...) printk(KERN_INFO DRIVER_PREFIX ": " _fmt, ##_arg)
#else
#define VSYNC_DEBUG(_fmt, _arg...)
#endif
...
#define MRSTLFB_COMMAND_COUNT		2
...
static void MRSTLFBCheckDisplayMode(MRSTLFB_DEVINFO *psDevInfo)
{
	struct drm_device *dev = psDevInfo->psDrmDevice;
	struct drm_crtc *crtc;
	unsigned int display_count = 0;
	unsigned int offset = 0;
	MRST_BOOL extended_mode = MRST_FALSE;

	psDevInfo->bIsCloneMode = MRST_FALSE;

	list_for_each_entry(crtc, &dev->mode_config.crtc_list, head)
	{
		if (drm_helper_crtc_in_use(crtc)) 
		{
			unsigned int crtc_offset = (crtc->y * crtc->fb->pitches[0] + crtc->x * (crtc->fb->bits_per_pixel / 8));
			struct psb_intel_crtc *psb_intel_crtc = to_psb_intel_crtc(crtc);

			if (psb_intel_crtc->pipe < MAX_PIPES)
			{
				psDevInfo->aui32CurrentOffset[psb_intel_crtc->pipe] = crtc_offset;
			}
			if (display_count > 0 && crtc_offset != offset)
			{
				extended_mode = MRST_TRUE;
			}
			offset = crtc_offset;
			display_count++;
		}
	...
	if (display_count > 1 && !extended_mode)
	{
		psDevInfo->bIsCloneMode = MRST_TRUE;
	}
	psDevInfo->bDisplayModeChecked = MRST_TRUE;
}

static void MRSTLFBFlip(MRSTLFB_DEVINFO *psDevInfo, MRSTLFB_BUFFER *psBuffer, int stride, int pipe)
{
	unsigned long ulAddr = (unsigned long)psBuffer->sDevVAddr.uiAddr;
	unsigned int offset = 0;
	MRST_BOOL isSystem = (psBuffer == &psDevInfo->sSystemBuffer ? MRST_TRUE : MRST_FALSE);
	...
	if (!psDevInfo->bDisplayModeChecked || isSystem)
	{
		MRSTLFBCheckDisplayMode(psDevInfo);
	}

	if (isSystem)
	{
		stride = psDevInfo->sDisplayDim.ui32ByteStride;
		offset = psDevInfo->aui32CurrentOffset[pipe];
	}

	if (!psDevInfo->bSuspended && !psDevInfo->bLeaveVT)
	{
		MRSTLFBFlipToSurface(psDevInfo, ulAddr, stride, offset, pipe);
		if (psDevInfo->bIsCloneMode && pipe == CRTC_PIPEA)
		{
			MRSTLFBFlipToSurface(psDevInfo, ulAddr, stride, offset, CRTC_PIPEB);
		}
	}

	psDevInfo->asLastFlips[pipe].ulLastFlipAddr = ulAddr;
	psDevInfo->asLastFlips[pipe].i32Stride = stride;
	psDevInfo->asLastFlips[pipe].ui32Offset = offset;
	psDevInfo->asLastFlips[pipe].bLastFlipAddrValid = MRST_TRUE;
}
...
static void MRSTLFBRestoreLastFlip(MRSTLFB_DEVINFO *psDevInfo)
{
	...
	if (!psDevInfo->bSuspended && !psDevInfo->bLeaveVT)
	{
		...
		{
			if (psDevInfo->asLastFlips[i].bLastFlipAddrValid)
			{
				MRSTLFBFlipToSurface(psDevInfo, psDevInfo->asLastFlips[i].ulLastFlipAddr, psDevInfo->asLastFlips[i].i32Stride, psDevInfo->asLastFlips[i].ui32Offset, i);
			}
		}
	}
}

static void MRSTLFBClearSavedFlip(MRSTLFB_DEVINFO *psDevInfo, int pipe)
{
	if (pipe < MAX_PIPES)
	{
		psDevInfo->asLastFlips[pipe].bLastFlipAddrValid = MRST_FALSE;
	}
}
...
static void FlushInternalVSyncQueue(MRSTLFB_SWAPCHAIN *psSwapChain, MRST_BOOL bFlip)
{
	...
	for(i = 0; i < psSwapChain->ulSwapChainLength; i++)
	{
		...
		if(psFlipItem->bFlipped == MRST_FALSE)
		{
			if (bFlip)
			{
				MRSTLFBFlip(psSwapChain->psDevInfo, psFlipItem->psBuffer, psSwapChain->ui32ByteStride, psSwapChain->ui32Pipe);
			}
		}
...
static void AddVSyncRequirement(MRSTLFB_DEVINFO *psDevInfo, IMG_UINT32 ui32Pipe, IMG_UINT32 ui32Reason)
{
	if (!psDevInfo->abVSyncEventRequired[ui32Pipe])
	{
		MRSTLFBEnableVSyncInterrupt(psDevInfo, ui32Pipe);
	}
	psDevInfo->abVSyncEventRequired[ui32Pipe] |= ui32Reason;
}

static void ReleaseVSyncRequirement(MRSTLFB_DEVINFO *psDevInfo, IMG_UINT32 ui32Pipe, IMG_UINT32 ui32Reason)
{
	psDevInfo->abVSyncEventRequired[ui32Pipe] &= ~ui32Reason;
	if (!psDevInfo->abVSyncEventRequired[ui32Pipe])
	{
		MRSTLFBDisableVSyncInterrupt(psDevInfo, ui32Pipe);
	}
}

static void FlushVSyncDependencies(MRSTLFB_DEVINFO *psDevInfo)
{
	IMG_UINT32 i;
	for ( i = 0 ; i < psDevInfo->ui32VSyncDependencies ; i++ )
	{
		MRSTLFB_VSYNC_DEPENDENCY_ITEM* pDepItem = &psDevInfo->asVSyncDependencies[i];
		if (pDepItem->bValid)
		{
			pDepItem->bValid = MRST_FALSE;
			psDevInfo->sPVRJTable.pfnPVRSRVCmdComplete((IMG_HANDLE)pDepItem->hCmdComplete, MRST_TRUE);
		}
	}
	psDevInfo->ui32VSyncDependencies = 0;
	ReleaseVSyncRequirement(psDevInfo, CRTC_PIPEA, VSYNC_REQUIRED_DEP);
	ReleaseVSyncRequirement(psDevInfo, CRTC_PIPEB, VSYNC_REQUIRED_DEP);
}

static MRSTLFB_SWAPCHAIN* GetSwapChainForPipe(MRSTLFB_DEVINFO *psDevInfo, int pipe)
{
	if (psDevInfo && pipe < MAX_PIPES)
	{
		return psDevInfo->apsCurrentSwapChain[pipe];
	}
	return NULL;
}

static void SetCurrentSwapChain(MRSTLFB_DEVINFO *psDevInfo, MRSTLFB_SWAPCHAIN *pSwapChain)
{
	if (psDevInfo && pSwapChain && pSwapChain->ui32Pipe < MAX_PIPES)
	{
		psDevInfo->apsCurrentSwapChain[pSwapChain->ui32Pipe] = pSwapChain;
	}
}

static void ClearCurrentSwapChain(MRSTLFB_DEVINFO *psDevInfo, int pipe)
{
	if (psDevInfo && pipe < MAX_PIPES)
	{
		psDevInfo->apsCurrentSwapChain[pipe] = NULL;
	}
}
...
{
	int i = 0;
	MRSTLFB_SWAPCHAIN* psCurrentSwapChain = NULL;

	if(psSwapChain != NULL) 
	{
		psCurrentSwapChain = GetSwapChainForPipe(psDevInfo, psSwapChain->ui32Pipe);
		if (psCurrentSwapChain && psCurrentSwapChain != psSwapChain)
			FlushInternalVSyncQueue(psCurrentSwapChain, MRST_FALSE);
		SetCurrentSwapChain(psDevInfo, psSwapChain);
		MRSTLFBFlip(psDevInfo, psBuffer, psSwapChain->ui32ByteStride, psSwapChain->ui32Pipe);
	}
	else
	{
		for (i = 0; i < MAX_PIPES; i++)
		{
			psCurrentSwapChain = GetSwapChainForPipe(psDevInfo, i);
			if (psCurrentSwapChain)
			{
				ClearCurrentSwapChain(psDevInfo, i);
				MRSTLFBFlip(psDevInfo, psBuffer, psDevInfo->sDisplayDim.ui32ByteStride, i);
			}
		}
	}
}
                                        ...
                                        MRST_BOOL bFlushState)
{
	...
	if (bFlushState)
	{
		if (psDevInfo->ulSetFlushStateRefCount == 0)
		{
			...
			psCurrentSwapChain = GetSwapChainForPipe(psDevInfo, CRTC_PIPEA);
			if (psCurrentSwapChain != NULL)
			{
				FlushInternalVSyncQueue(psCurrentSwapChain, MRST_TRUE);
			}
			psCurrentSwapChain = GetSwapChainForPipe(psDevInfo, CRTC_PIPEB);
			if (psCurrentSwapChain != NULL)
			{
				FlushInternalVSyncQueue(psCurrentSwapChain, MRST_TRUE);
			}
			FlushVSyncDependencies(psDevInfo);
		}
                                      ...
                                      IMG_UINT32 *pui32SwapChainID)
{
	...
	psDevInfo->bDisplayModeChecked = MRST_FALSE;
	
	if (ui32Flags & PVRSRV_CREATE_SWAPCHAIN_CRTC_PIPEA)
	{
		pipe = CRTC_PIPEA;
	}
	else if (ui32Flags & PVRSRV_CREATE_SWAPCHAIN_CRTC_PIPEB)
	{
		pipe = CRTC_PIPEB;
	}
	
	psDrmDev = psDevInfo->psDrmDevice;
	list_for_each_entry(crtc, &psDrmDev->mode_config.crtc_list, head)  
	{
		struct psb_intel_crtc *psb_intel_crtc = to_psb_intel_crtc(crtc);
		if (psb_intel_crtc && psb_intel_crtc->pipe == pipe)
		{
			if(psDstSurfAttrib->pixelformat != psDevInfo->sDisplayFormat.pixelformat
			|| psDstSurfAttrib->sDims.ui32Width != drm_mode_width(&crtc->mode)
			|| psDstSurfAttrib->sDims.ui32Height != drm_mode_height(&crtc->mode))
			{
	...
	psSwapChain->ui32Height = psDstSurfAttrib->sDims.ui32Height;
	psSwapChain->ui32ByteStride = psDstSurfAttrib->sDims.ui32ByteStride;
	psSwapChain->ui32Width = psDstSurfAttrib->sDims.ui32Width;
	psSwapChain->ui32Pipe = pipe;
	
	memset(ppsBuffer, 0, sizeof(MRSTLFB_BUFFER *) * ui32BufferCount);
	for (i = 0; i < ui32BufferCount; i++)
	{
		unsigned long bufSize = psDstSurfAttrib->sDims.ui32ByteStride * psDstSurfAttrib->sDims.ui32Height;
	...
	psCurrentSwapChain = GetSwapChainForPipe(psDevInfo, pipe);
	if (psCurrentSwapChain == NULL)
		SetCurrentSwapChain(psDevInfo, psSwapChain);

	psDevInfo->ui32SwapChainNum++;
	AddVSyncRequirement(psDevInfo, psSwapChain->ui32Pipe, VSYNC_REQUIRED_SWAPCHAIN);
	...
	IMG_HANDLE hSwapChain)
{
	...
	ReleaseVSyncRequirement(psDevInfo, psSwapChain->ui32Pipe, VSYNC_REQUIRED_SWAPCHAIN);
	...
	if (psDevInfo->ui32SwapChainNum == 0)
	{
		...
		MRSTLFBClearSavedFlip(psDevInfo, CRTC_PIPEA);
		MRSTLFBClearSavedFlip(psDevInfo, CRTC_PIPEB);
	}

	psCurrentSwapChain = GetSwapChainForPipe(psDevInfo, psSwapChain->ui32Pipe);
	if (psCurrentSwapChain == psSwapChain)
		ClearCurrentSwapChain(psDevInfo, psSwapChain->ui32Pipe);
...
static MRST_BOOL MRSTLFBCheckVSyncDependencies(MRSTLFB_DEVINFO *psDevInfo, IMG_UINT32 ui32Pipe)
{
	IMG_UINT32 i;
	unsigned long ulLockFlags;
	IMG_UINT32 remainingDeps = 0;
	int done = 0;
	...
	psDevInfo->ui32VSyncDependencies = remainingDeps;
#if (ENABLE_VSYNC_DEBUG)
	if (done || psDevInfo->abVSyncEventRequired[ui32Pipe] & VSYNC_REQUIRED_DEP)
	{
		if (done)
		{
			VSYNC_DEBUG("VSYNC Complete Pipe %u\n", ui32Pipe);
		}
		else
		{
			VSYNC_DEBUG("VSYNC Pipe %u\n", ui32Pipe);
		}
	}
#endif
	ReleaseVSyncRequirement(psDevInfo, ui32Pipe, VSYNC_REQUIRED_DEP);
	spin_unlock_irqrestore(&psDevInfo->sSwapChainLock, ulLockFlags);
	return MRST_TRUE;
}

static MRST_BOOL MRSTLFBVSyncIHandler(MRSTLFB_DEVINFO *psDevInfo, int pipe)
{
	...
	psSwapChain = GetSwapChainForPipe(psDevInfo, pipe);
	if (psSwapChain == NULL)
	{
		goto ExitUnlock;
	}
...
MRSTLFBVSyncISR(struct drm_device *psDrmDevice, int iPipe)
{
	...
	MRSTLFBCheckVSyncDependencies(psDevInfo, iPipe);
	MRSTLFBVSyncIHandler(psDevInfo, iPipe);
...
static IMG_BOOL ProcessVSync(IMG_HANDLE  hCmdCookie,
                            IMG_UINT32  ui32DataSize,
                            IMG_VOID   *pvData)
{
	DISPLAYCLASS_VSYNC_COMMAND *psVSyncCmd;
	MRSTLFB_DEVINFO *psDevInfo;
	unsigned long ulLockFlags;
	IMG_UINT32 i;
	...
	{
		printk(KERN_WARNING DRIVER_PREFIX
			": ProcessVSync - Invalid params\n");
		return IMG_FALSE;
	}

	psVSyncCmd = (DISPLAYCLASS_VSYNC_COMMAND*)pvData;
	psDevInfo = (MRSTLFB_DEVINFO*)psVSyncCmd->hExtDevice;
	spin_lock_irqsave(&psDevInfo->sSwapChainLock, ulLockFlags);

	VSYNC_DEBUG("ProcessVSync Pipe %u\n", psVSyncCmd->ui32Pipe);
	if(psDevInfo->bFlushCommands || psDevInfo->bSuspended)
	{
		VSYNC_DEBUG("ProcessVSync Not Ready (flush %u, suspended %u)\n", psDevInfo->bFlushCommands, psDevInfo->bSuspended);
		psDevInfo->sPVRJTable.pfnPVRSRVCmdComplete(hCmdCookie, IMG_TRUE);
		goto ExitTrueUnlock;
	}

	if (!MRSTLFBIsPipeEnabled(psDevInfo, psVSyncCmd->ui32Pipe))
	{
		VSYNC_DEBUG("ProcessVSync pipe disabled\n");
		psDevInfo->sPVRJTable.pfnPVRSRVCmdComplete(hCmdCookie, IMG_TRUE);
		goto ExitTrueUnlock;
	}

	for ( i = 0 ; i < MAX_VSYNC_DEPENDENCIES ; i++ )
	{
		if (!psDevInfo->asVSyncDependencies[i].bValid)
		{
			psDevInfo->asVSyncDependencies[i].hCmdComplete = hCmdCookie;
			psDevInfo->asVSyncDependencies[i].bValid = MRST_TRUE;
			psDevInfo->asVSyncDependencies[i].ui32Pipe = psVSyncCmd->ui32Pipe;
			if ( i >= psDevInfo->ui32VSyncDependencies)
			{
				psDevInfo->ui32VSyncDependencies = i + 1;
			}
			break;
		}
	}

	if ( i == MAX_VSYNC_DEPENDENCIES )
	{
		printk(KERN_WARNING DRIVER_PREFIX
			": ProcessVSync - too many dependencies waiting\n");
		psDevInfo->sPVRJTable.pfnPVRSRVCmdComplete(hCmdCookie, IMG_TRUE);
		goto ExitTrueUnlock;
	}

	AddVSyncRequirement(psDevInfo, psVSyncCmd->ui32Pipe, VSYNC_REQUIRED_DEP);
...
void MRSTLFBSuspend(void)
{
	...
	VSYNC_DEBUG("MRSTLFBSuspend\n");
	if (!psDevInfo->bSuspended)
	{
#if !defined(PVR_MRST_STYLE_PM)
		MRSTLFBDisableVSyncInterrupt(psDevInfo, CRTC_PIPEA);
		MRSTLFBDisableVSyncInterrupt(psDevInfo, CRTC_PIPEB);
#endif
...
void MRSTLFBResume(void)
{
	...
	spin_lock_irqsave(&psDevInfo->sSwapChainLock, ulLockFlags);
	VSYNC_DEBUG("MRSTLFBResume\n");

	if (psDevInfo->bSuspended)
	{
#if !defined(PVR_MRST_STYLE_PM)
		if (psDevInfo->abVSyncEventRequired[CRTC_PIPEA])
		{
			MRSTLFBEnableVSyncInterrupt(psDevInfo, CRTC_PIPEA);
		}
		if (psDevInfo->abVSyncEventRequired[CRTC_PIPEB])
		{
			MRSTLFBEnableVSyncInterrupt(psDevInfo, CRTC_PIPEB);
		}
#endif
...
static MRST_ERROR InitDev(MRSTLFB_DEVINFO *psDevInfo)
{
	 ...
	 */
	psDrmPrivate->ui32MainPipe = MRSTLFBFindMainPipe(psDevInfo->psDrmDevice);
...
MRST_ERROR MRSTLFBInit(struct drm_device * dev)
{
	...
	if (psDevInfo == NULL)
	{
		...
		memset(psDevInfo->apsCurrentSwapChain, 0, sizeof(psDevInfo->apsCurrentSwapChain));
		memset(psDevInfo->asLastFlips, 0, sizeof(psDevInfo->asLastFlips));
		psDevInfo->bDisplayModeChecked = MRST_FALSE;
		psDevInfo->bFlushCommands = MRST_FALSE;
		...
		psDevInfo->ui32FlipEventEnabled = 0;
	...
	pfnCmdProcList[DC_VSYNC_COMMAND] = ProcessVSync;
	aui32SyncCountList[DC_VSYNC_COMMAND][0] = 0;
	aui32SyncCountList[DC_VSYNC_COMMAND][1] = 2;
...

{
	...
	*ppBuffer = MRSTLFBAllocKernelMem( sizeof( MRSTLFB_BUFFER ) );
	...
	ret = psb_gtt_map_pvr_memory(dev, (unsigned long)*ppBuffer,
...
int MRSTLFBFreeBuffer(struct drm_device *dev, MRSTLFB_BUFFER **ppBuffer)
{
	...
	psb_gtt_unmap_pvr_memory(dev, (unsigned long)*ppBuffer,
				 (*ppBuffer)->ui32OwnerTaskID);
...
unsigned long MRSTLFBGetPhysPage(MRSTLFB_BUFFER *pBuffer, int index)
{
	if (pBuffer->bIsContiguous)
	{
		return pBuffer->uSysAddr.sCont.uiAddr + (PAGE_SIZE * index);
	}
	else
	{
		return pBuffer->uSysAddr.psNonCont[index].uiAddr;
	}
}
